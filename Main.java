...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("----------------------------------------------------");
        System.out.println("请输入需要监控的课程索引:");
        int index = Integer.parseInt(sc.next());
        System.out.println("监控速度(秒数):");
        int speed = Integer.parseInt(sc.next()) * 1000;
        System.out.println("监控的课程是: " + courseList.get(index).get("name") + "--课程ID: " + courseList.get(index).get("courseId") + "--班级ID: " + courseList.get(index).get("classId"));
        System.out.println("输入1开始执行,输入字符结束!");
        String flag = sc.next();
        sc.close();
    ...
    private static void StartTask(int index, int speed, List<Map<String, String>> courseDate) {
        ...
        System.out.println("当前监控课程: " + courseDate.get(index).get("name"));
    ...
    private static void sign(String apID) {
        System.out.println("【签到】已检测到签到任务,开始签到!");
